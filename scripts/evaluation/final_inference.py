...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    if loop or interp:
        ...
        _img_1 = slerp(img_1,img_2,0.5)
        _img_2 = slerp(img_2,img_1,0.5)
        
        img = torch.cat([_img_1,_img_2],dim=0)
    ...
    for prompt in prompts:
        ...
        cond_emb_full,cond_emb_inverse, cond_emb_1,cond_emb_2=None, None, None, None
    ...
    if unconditional_guidance_scale != 1.0:
        ...
        uc_img_emb = model.embedder(torch.zeros_like(torch.cat([img_1,img_2]))) ## b l c
...
def run_inference(args, gpu_num, gpu_no):
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            samples_2 = torch.flip(batch_samples[1], dims=[2])
            pre_frame = first_frame
            for frame_idx in range(1, n_frames-2):
                cur_frame_1 = samples_1[0,:,frame_idx]
                dist_1 = 0.8*perceptual_loss(cur_frame_1.unsqueeze(0), pre_frame)+ 0.2*perceptual_loss(cur_frame_1.unsqueeze(0), first_frame)
                cur_frame_2 = samples_2[0,:,frame_idx]
                dist_2 = 0.8*perceptual_loss(cur_frame_2.unsqueeze(0), pre_frame)+ 0.2*perceptual_loss(cur_frame_2.unsqueeze(0), first_frame)
                samples_1[0,:,frame_idx] = cur_frame_1 if dist_1<dist_2 else cur_frame_2
                pre_frame = samples_1[0,:,frame_idx].unsqueeze(0)
            # previous_frame = 
            # for (frame_1, frame_2)
