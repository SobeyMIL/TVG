...
from lvdm_tvg.common import (
    checkpoint,
    exists,
    default,
)
from lvdm_tvg.basics import zero_module
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context=None, mask=None):
        weights = 0.9
        _x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None, mask=mask) + weights*x
        ...
        if _x.shape[0]<_x.shape[1]:
            ...
            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_first_half[0,:,:], y_train=x_first_half[-1,:,:])
            # x_first_half[2:num_frames-1,:,:] = gp.predict(x_first_half[1:num_frames-2,:,:])

            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_second_half[0,:,:], y_train=x_second_half[-1,:,:])
            # x_second_half[2:num_frames-1,:,:] = gp.predict(x_second_half[1:num_frames-2,:,:])
            ...
            # 相加反序后的两部分
            alpha_list = torch.linspace(0.9, 0.1, half_size).view(-1,1,1).to(x.device)
            new_first_half = alpha_list*x_first_half + torch.flip(alpha_list,dims=[0])* x_second_half_reversed
            new_second_half = alpha_list*x_second_half + torch.flip(alpha_list,dims=[0])* x_first_half_reversed
            ...
            half_size = _x.shape[0] // 2
            x_first_half = x[ :half_size,:, :]#torch.Size([4096, 16, 512])
            x_second_half = x[ half_size:,:, :]

            gp = GaussianProcess(kernel=rbf_kernel, X_train=x_first_half[:,0,:], y_train=x_first_half[:,-1,:])
            x_first_half[:,1:num_frames-2,:] = gp.predict(x_first_half[:,1:num_frames-2,:].transpose(1,0)).transpose(1,0).contiguous()#torch.Size([4096, 16, 512])

            gp = GaussianProcess(kernel=rbf_kernel, X_train=x_second_half[:,0,:], y_train=x_second_half[:,-1,:])
            x_second_half[:,1:num_frames-2,:] = gp.predict(x_second_half[:,1:num_frames-2,:].transpose(1,0)).transpose(1,0).contiguous()
            
            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_first_half[:,0,:], y_train=x_first_half[:,-1,:])
            # x_first_half[:,2:num_frames-1,:] = gp.predict(x_first_half[:,1:num_frames-2,:].transpose(1,0)).transpose(1,0).contiguous()

            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_second_half[:,0,:], y_train=x_second_half[:,-1,:])
            # x_second_half[:,2:num_frames-1,:] = gp.predict(x_second_half[:,1:num_frames-2,:].transpose(1,0)).transpose(1,0).contiguous()
            #时序传播
            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_first_half[:,0,:], y_train=x_first_half[:,1,:])
            ...
            alpha_list = torch.linspace(0.9, 0.1, num_frames).view(1,-1,1).to(x.device)
            new_first_half = alpha_list*x_first_half + torch.flip(alpha_list,dims=[1])* x_second_half_reversed
            new_second_half = alpha_list*x_second_half + torch.flip(alpha_list,dims=[1])* x_first_half_reversed
            x=torch.cat([new_first_half,new_second_half],dim=0)
        ...
        _x = self.attn2(self.norm2(x), context=context, mask=mask) + weights*x

        if _x.shape[0]<_x.shape[1]:
            num_frames=_x.shape[0]
            half_size = _x.shape[0] // 2
            num_frames=half_size#应该为16，不是32
            x_first_half = x[:half_size]
            x_second_half = x[half_size:]
            gp = GaussianProcess(kernel=rbf_kernel, X_train=x_first_half[0,:,:], y_train=x_first_half[-1,:,:])
            x_first_half[1:num_frames-2,:,:] = gp.predict(x_first_half[1:num_frames-2,:,:])
            ...
            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_first_half[0,:,:], y_train=x_first_half[-1,:,:])
            # x_first_half[1:num_frames-1,:,:] = gp.predict(x_first_half[1:num_frames-1,:,:])

            # gp = GaussianProcess(kernel=rbf_kernel, X_train=x_second_half[0,:,:], y_train=x_second_half[-1,:,:])
            # x_second_half[1:num_frames-1,:,:] = gp.predict(x_second_half[1:num_frames-1,:,:])
